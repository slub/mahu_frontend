 ...
 * 
 */
class SupplementEditorPage extends AbstractPage {
	...
	addListeners() {
		...
		$("#fabricators select.chosen-container").chosen();
		...
		$("#addFabricatorBtn").click(function(ev){
			let num = $("#fabricators").find(".cloneable").length;
			
			let template = $("#involvedTemplate > div").clone();
			
			let clonedInputElements = $(template.find("input, textarea, select"));
			for (let i=0; i < clonedInputElements.length; ++i) {
				let clonedInputElement = clonedInputElements[i];
			...
			// add new element to the DOM
			$("#fabricators").append(template);
			
			template.find("select.chosen-container").chosen();
		...
		setInterval(function(){
			...
			let a = form.action.replace("tx_mahufrontend_supplementeditor%5Baction%5D=edit","tx_mahufrontend_supplementeditor%5Baction%5D=autosave");
			a = a.replace("tx_mahufrontend_supplementeditor%5Baction%5D=new","tx_mahufrontend_supplementeditor%5Baction%5D=autosave");
			fetch(a + "&tx_mahufrontend_supplementeditor[format]=data&type=1369315165", {
				method: form.method,
